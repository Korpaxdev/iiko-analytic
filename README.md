...
- Управление несколькими серверами iiko (добавление, редактирование, удаление, переключение)
- Построение графиков (столбчатая, линейная, круговая диаграммы) на основе полученных OLAP-данных
- Автоматическая вторая ось Y при разных масштабах данных
- Импорт JSON-конфигурации отчета
- Генерация SHA-1 хэша пароля
- Экспорт результатов в Excel
- Поддержка темной темы
- Сохранение серверов и настроек в localStorage
- Автоматическая миграция с одиночного подключения на мультисерверный формат

**Особенности:**

- Модульная структура JavaScript (11 отдельных модулей)
...
- `utils.js` - утилиты (тема, debounce, форматирование)
- `servers.js` - управление серверами (CRUD, localStorage, модал, миграция)
...
- `query.js` - выполнение и конструирование запросов, таблица результатов, сортировка, фильтрация, экспорт в Excel
- `json-highlight.js` - подсветка синтаксиса JSON
- `import.js` - импорт JSON-конфигурации отчета
- `chart.js` - построение графиков (Chart.js) с автокомплитом выбора полей и двойной осью Y
- `hash.js` - генерация SHA-1 хэша пароля
